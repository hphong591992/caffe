...
class SoftmaxWithMaxMarginLossLayerTest : public MultiDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;

 protected:
  SoftmaxWithMaxMarginLossLayerTest()
      : blob_bottom_data_(new Blob<Dtype>(10, 5, 2, 3)),
        blob_bottom_label_(new Blob<Dtype>(10, 1, 2, 3)),
        blob_bottom_aux_data_(new Blob<Dtype>(10, 5, 2, 3)),
        blob_bottom_similarity_(new Blob<Dtype>(10, 1, 2, 3)),
    ...
    filler.Fill(this->blob_bottom_aux_data_);
    for (int i = 0; i < blob_bottom_label_->count(); ++i) {
      blob_bottom_label_->mutable_cpu_data()[i] = caffe_rng_rand() % 5;
      blob_bottom_similarity_->mutable_cpu_data()[i] = caffe_rng_rand() % 2;
    }
    blob_bottom_vec_.push_back(blob_bottom_data_);
    blob_bottom_vec_.push_back(blob_bottom_label_);
    blob_bottom_vec_.push_back(blob_bottom_aux_data_);
    blob_bottom_vec_.push_back(blob_bottom_similarity_);
    blob_top_vec_.push_back(blob_top_loss_);
  }
  virtual ~SoftmaxWithMaxMarginLossLayerTest() {
  ...
  Blob<Dtype>* const blob_bottom_aux_data_;
  Blob<Dtype>* const blob_bottom_similarity_;
  Blob<Dtype>* const blob_top_loss_;
  vector<Blob<Dtype>*> blob_bottom_vec_;
  vector<Blob<Dtype>*> blob_top_vec_;
};

TYPED_TEST_CASE(SoftmaxWithMaxMarginLossLayerTest, TestDtypesAndDevices);

TYPED_TEST(SoftmaxWithMaxMarginLossLayerTest, TestGradient) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  layer_param.add_loss_weight(1);
  layer_param.mutable_softmax_with_max_margin_loss_param()->set_lambda(1);
  layer_param.mutable_softmax_with_max_margin_loss_param()->set_margin(0.1);
  SoftmaxWithMaxMarginLossLayer<Dtype> layer(layer_param);
