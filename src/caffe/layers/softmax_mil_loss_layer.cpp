...
void SoftmaxWithMILLossLayer<Dtype>::LayerSetUp(
...
template <typename Dtype>
void SoftmaxWithMILLossLayer<Dtype>::Reshape(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void SoftmaxWithMILLossLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  const Dtype* selector_indicator = bottom[3]->cpu_data();
  ...
  for (int i = 0; i < outer_num_; ++i) {
    for (int j = 0; j < inner_num_; j++) {
      ...
      const int sim = static_cast<int>(selector_indicator[i * dim + label_value * inner_num_ + j]) == (i * dim + label_value * inner_num_ + j);
...
template <typename Dtype>
void SoftmaxWithMILLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
    ...
    for (int i = 0; i < outer_num_; ++i) {
      for (int j = 0; j < inner_num_; ++j) {
        ...
        if (has_ignore_label_ && label_value == ignore_label_) {
          ...
          if (sim == 0) {
            const Dtype sample_prob = bottom_diff[i * dim + label_value * inner_num_ + j];
            const Dtype aux_sample_prob = aux_bottom_diff[i * dim + label_value * inner_num_ + j];
...
#ifdef CPU_ONLY
STUB_GPU(SoftmaxWithMILLossLayer);
#endif

INSTANTIATE_CLASS(SoftmaxWithMILLossLayer);
REGISTER_LAYER_CLASS(SoftmaxWithMILLoss);
