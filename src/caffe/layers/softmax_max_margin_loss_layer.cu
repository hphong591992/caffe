...
__global__ void SoftmaxMaxMarginLossForwardGPU(const int nthreads,
          const Dtype* prob_data, const Dtype* aux_prob_data, const Dtype* label, const Dtype* similarity, Dtype* loss,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    const int similarity_value = static_cast<int>(similarity[n * spatial_dim + s]);
...
template <typename Dtype>
void SoftmaxWithMaxMarginLossLayer<Dtype>::Forward_gpu(
  ...
  const Dtype* similarity = bottom[3]->gpu_data();
  ...
  SoftmaxMaxMarginLossForwardGPU<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
      CAFFE_CUDA_NUM_THREADS>>>(nthreads, prob_data, aux_prob_data, label, similarity, loss_data, lambda, margin,
...
template <typename Dtype>
__global__ void SoftmaxMaxMarginLossBackwardGPU(const int nthreads, const Dtype* top,
          const Dtype* label, const Dtype* similarity, Dtype* bottom_diff, Dtype* aux_bottom_diff, const Dtype lambda, const Dtype margin,
...
template <typename Dtype>
void SoftmaxWithMaxMarginLossLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    Dtype* counts = prob_.mutable_gpu_diff();
    // NOLINT_NEXT_LINE(whitespace/operators)
    SoftmaxMaxMarginLossBackwardGPU<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
        CAFFE_CUDA_NUM_THREADS>>>(nthreads, top_data, label, similarity, bottom_diff, aux_bottom_diff, lambda, margin,
        outer_num_, dim, inner_num_, has_ignore_label_, ignore_label_, counts);
...
INSTANTIATE_LAYER_GPU_FUNCS(SoftmaxWithMaxMarginLossLayer);
